"""
Phase 1.1 prototype — standalone inference worker.

Pulls one RTSP stream from the MediaMTX prototype, decodes with PyAV
h264_cuvid (NVDEC), applies the same motion gate the production code uses,
calls Triton for YOLO-World inference, and prints detection events.

This is the seed of the future Case-A inference worker.  Deliberately:
  - one camera (no multi-cam yet — that's Phase 2 with CameraRuntime)
  - no FastAPI, no clip recording, no per-camera state machine
  - prints events instead of broadcasting them
  - calls Triton over HTTP with tritonclient.http (matches production)
...
Run inside the bench container (see docker-compose.test.yml):
  docker compose -f docker-compose.yml -f docker-compose.test.yml --profile bench \\
      run --rm bench /bench/inference_prototype.py \\
          --rtsp rtsp://bianca-mediamtx-prototype:8554/test \\
          --triton bianca-triton:8002 \\
          --duration 60

The prototype is intentionally self-contained — once Phase 1 commits to the
swap, the inference worker container just runs this module as its entrypoint.
"""
