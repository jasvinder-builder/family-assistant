...
- `scripts/test_deepstream_multicam.py` — 2 simultaneous file sources via single DeepStream pipeline
...
- Removed `current_url` context from `/cameras` page (multi-camera has no single URL)
...
- All existing camera routes unchanged (backward-compatible)
...
- Old `/cameras/set-stream` endpoint retained for any existing integrations
...
- `scripts/test_deepstream_multicam.py` — Phase 5 multi-camera test
